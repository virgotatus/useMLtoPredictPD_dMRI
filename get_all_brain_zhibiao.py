...
#封装  还有控制病人要变，只for了一个
namelist = [patient3_name_list,control3_name_list]
foldlist = ['patient','control']
for c in range(2):
    for f_name in namelist[c]:
        dipy_home = pjoin(os.path.expanduser('~'), 'dipy_data')
        folder = pjoin(dipy_home, foldlist[c])
        fraw = pjoin(folder, f_name+'.nii.gz')
        fbval = pjoin(folder, f_name+'.bval')
        fbvec = pjoin(folder, f_name+'.bvec')
        flabels = pjoin(folder, f_name+'.nii-label.nii.gz')
        ...
        print rtop_signal.shape
        rtop_signal_img = nib.Nifti1Image(rtop_signal.astype(np.float32), affine)
        nib.save(rtop_signal_img, os.getcwd()+'/zhibiao/'+f_name+'_rtop_signal.nii.gz')
        
        """rtop_pdf"""
        print('Calculating... rtop_pdf')
        rtop_pdf = asmfit.rtop_pdf()
        rtop_pdf[np.isnan(rtop_pdf)] = 0
        #print GFA[:,:,slice].T
        print('Saving rtop_pdf_img.png')
        print rtop_pdf.shape
        rtop_pdf_img = nib.Nifti1Image(rtop_pdf.astype(np.float32), affine)
        nib.save(rtop_pdf_img, os.getcwd()+'/zhibiao/'+f_name+'_rtop_pdf.nii.gz')
