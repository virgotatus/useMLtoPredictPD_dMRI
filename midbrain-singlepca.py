...
out = estimator.transform(X[1])
cc = np.zeros([1,380])
for i in range(6):
    cc += out[0][i] * components_[i]

vmax = max(cc.max(), -cc.min())
plt.figure(3)
plt.imshow(cc.reshape(image_shape), cmap=plt.cm.gray,
                   interpolation='nearest',
                   vmin=-vmax, vmax=vmax)
...
for i in range(6):
    for j in range(380):
        if components_[i][j]>0.1:
            a[i][j]=components_[i][j]
